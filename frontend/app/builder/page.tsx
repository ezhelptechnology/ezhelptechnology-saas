            ...
            {stepsUi.map((s, idx) => {
              ...
              return (
                <div key={s.id} className="flex items-center gap-2">
                  <div className={`flex h-7 w-7 items-center justify-center rounded-full border text-xs font-semibold ${isActive ? "border-amber-400 bg-amber-400 text-black shadow shadow-amber-500/40"
              ...
              <div className="h-[460px] overflow-y-auto p-6 space-y-4">
                {messages.map((msg, i) => (
                  <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap ${msg.role === "user" ? "bg-amber-400 text-black" : "bg-slate-800 text-slate-50"
                      }`}>
                      {msg.content}
                    </div>
                  </div>
          ...
          <section className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {[
                ...
                return (
                  <div key={agent.key} className={`rounded-2xl border p-6 transition-all ${status === "working" ? "border-amber-400 bg-amber-500/5"
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {[
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition ${previewTab === tab.id ? "bg-amber-400 text-black" : "bg-slate-800 text-slate-300 hover:bg-slate-700"
                    }`}
                >
                  {tab.label}
                </button>
                      ...
                      <div className="grid md:grid-cols-2 gap-6 mt-6">
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {(Array.isArray(design.voice) ? design.voice : design.voice?.tone || ["professional", "friendly"]).map((v: string, i: number) => (
                      ...
                      <div className="grid md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <ul className="space-y-2">
                            {(final.clientPortalFeatures || []).map((f: any, i: number) => (
                              <li key={i} className="flex items-center gap-2 text-sm text-slate-300">
                                <span className="text-sky-400">•</span> {typeof f === 'string' ? f : f.name}
                              </li>
                            ))}
                          </ul>
                        </div>
                        <div>
                          ...
                          <ul className="space-y-2">
                            {(final.adminDashboardFeatures || []).map((f: any, i: number) => (
                              <li key={i} className="flex items-center gap-2 text-sm text-slate-300">
                                <span className="text-violet-400">•</span> {typeof f === 'string' ? f : f.name}
                              </li>
                            ))}
                          </ul>
                        </div>
